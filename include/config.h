...
// --- Certificado raiz para validar TLS ---------------------------------
// Los tres hosts que usa el firmware (OpenSky, GNews, Open-Meteo) terminan
// en el mismo root: ISRG Root X1, el de Let's Encrypt. Se pinea el ROOT y
// no un certificado de hoja ni un intermedio a proposito: los de hoja
// rotan cada ~90 dias y los intermedios de tanto en tanto, pero el root
// tiene vigencia hasta 2035 (04-jun-2015 a 04-jun-2035) y con eso alcanza
// para no tener que tocar esto nunca mas.
//
// Se extrajo del bundle de CAs que trae Git para Windows
// (usr/ssl/certs/ca-bundle.crt) -un almacen de confianza LOCAL- y no de una
// conexion en vivo a los hosts: confiar en lo que devuelve una conexion
// hecha desde un entorno de desarrollo en la nube seria darle la razon a
// un posible intermediario en el camino, justo lo que esto viene a evitar.
// Confirmado funcionando en la practica: los tres hosts respondieron con
// el certificado validado, ya en la red de casa del dispositivo.
static const char* TLS_ROOT_CA_PEM =
R"CERT(
-----BEGIN CERTIFICATE-----
